...
func extractLessonNumberFromHeaderCell(cell string) int {
	...
	// Если в ячейке есть слово "пара" — извлекаем цифру 1..6 откуда угодно в тексте.
	if strings.Contains(text, "пара") {
		for _, r := range text {
			if r >= '1' && r <= '6' {
				return int(r - '0')
	...
	// Иначе в некоторых шаблонах номер пары в шапке бывает коротким (например "1", "1-я", "2к").
	// Ограничиваем по длине, чтобы не ловить случайные цифры из дат/итогов.
	runes := []rune(text)
	if len(runes) > 4 {
		return 0
	}
